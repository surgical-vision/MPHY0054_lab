...
class FKinePublisher(Node):
    def __init__(self):
        ...
        self.get_logger().info('✅ FKinePublisher node started (chain-style TF).')
    ...
    def fkine_callback(self, joint_msg):
        ...
        for i in range(n):
            theta_actual = theta[i] + joint_msg.position[i] + custom_offset[i]
            T_i = forward_kinematics(a[i], alpha[i], d[i], theta_actual)
            T_total = T_total @ T_i
            transform = TransformStamped()
            transform.header.stamp = self.get_clock().now().to_msg()
            transform.header.frame_id = parent_frame if i == 0 else link_names[i - 1]
