...
class YoubotKDL:
    def __init__(self, node: Node, kdl_kine_chain, tf_suffix):
        """Create a TF broadcaster, FK solver, and Jacobian solver for the YouBot arm.

        Parameters
        ----------
        node : rclpy.node.Node
            ROS 2 node that owns subscriptions, timers, and logging.
        kdl_kine_chain : PyKDL.Chain
            Kinematic chain describing the manipulator (e.g., built from the URDF).
        tf_suffix : str
            String appended to the child frame so multiple FK sources can coexist.

        Notes
        -----
        Two subscribers listen to the same ``/joint_states`` stream: one computes
        forward kinematics for TF broadcasting, the other evaluates the Jacobian.
        Both re-use the helper functions defined later in this module.
        """
        # Set class inputs
        self.node = node
        ...
        self.sub_fk = self.node.create_subscription(
            JointState,
            '/joint_states',
            self.fkine_wrapper,
            10
        ...
        self.sub_j = self.node.create_subscription(
            JointState,
            '/joint_states',
            self.print_jacobian,
            10
    ...
    def forward_kinematics(self, joints_readings):
        """Evaluate the end-effector pose for the supplied joint configuration.

        Parameters
        ----------
        joints_readings : Sequence[float]
            Joint angles (radians) ordered according to the KDL chain segments.

        Returns
        -------
        numpy.ndarray
            4x4 homogeneous transform mapping ``base_link`` onto ``tf_suffix_link``.
        """
    ...
    def print_jacobian(self, joint_msg: JointState):
        """Callback that logs the instantaneous geometric Jacobian.

        Parameters
        ----------
        joint_msg : sensor_msgs.msg.JointState
            JointState published by the GUI; we only use the ``position`` vector.
        """
        j = self.get_jacobian(joint_msg.position)
        self.node.get_logger().info(str(j))
    ...
    def get_jacobian(self, joints_readings):
        """Compute the 6xN spatial Jacobian expressed at the end-effector.
        ...
        -------
        numpy.matrix
            Matrix whose columns are twists ``[v; ω]`` describing the effect of
            each joint on the end-effector spatial velocity.

        Mathematical Notes
        ------------------
        PyKDL assembles the body Jacobian by differentiating the transformation
        product ``T_0^n = Π exp([S_i] q_i)``. Each column corresponds to the screw
        axis expressed in the base frame, propagated through preceding transforms.
        """
        joints_kdl = self.list_to_kdl_jnt_array(joints_readings)
        jacobian_kdl = kdl.Jacobian(self.kine_chain.getNrOfJoints())
    ...
    def fkine_wrapper(self, joint_msg):
        """JointState subscriber used to broadcast TF transforms.

        Parameters
        ----------
        joint_msg : sensor_msgs.msg.JointState
            Contains the 5 arm joint positions; velocities/efforts are ignored.
        """
    ...
    @staticmethod
    def rotmat2q(T):
        """Convert a 3x3 rotation matrix into a geometry_msgs Quaternion."""
    ...
    def convert_kdl_frame_to_mat(frame):
        """Transform a PyKDL Frame into a 4x4 homogeneous matrix."""
    ...
    def list_to_kdl_jnt_array(joints):
        """Convert an iterable of joints into a fixed-length KDL JntArray."""
    ...
    def convert_kdl_to_mat(data):
        """Convert KDL Jacobian/Matrix objects into ``numpy.matrix`` instances."""
...
def kine_chain_urdf(node: Node):
    """Build a PyKDL chain between ``base_link`` and ``arm_link_ee`` using the URDF."""
    ...
    kine_chain = build_kdl_chain_from_urdf(robot_description, "base_link", "arm_link_ee")
    ## Load in the URDF correctly from the robot description and create the kinematic chain##
...
def main():
    """Entry point: initialise the node, build the URDF chain, and spin."""
    rclpy.init()
    node = rclpy.create_node('youbot_KDL_forward_kinematic_node')
    ...
    urdf_chain = kine_chain_urdf(node)
    urdf_ = YoubotKDL(node, urdf_chain, 'urdf')
    ## Create the URDF_chain by calling the kine_chain_urdf function and then create the YoubotKDLKinematic class ##
